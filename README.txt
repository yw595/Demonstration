...
The script cleanModel.m takes this xml file as input to create a COBRA model variable, which it then outputs as New1122Specific.xml, and obtains flux distributions that result from maximizing flux through specific reactions, which are stored in 1122FluxMaps. cleanModel also uses the handcrafted files ChangedNamesRxns.txt and ChangedNamesMets.txt, to replace obscure names imported from 1122Specific.xml. You can run MATLAB scripts by opening a MATLAB command window, which I believe I installed on Junbo's computer, navigating to the folder with the script, and just typing the script's name and pressing Enter. Note that cleanModel runs some functions from the COBRA library. Therefore, you must the run the command initCobraToolbox in the MATLAB command window before running cleanModel. cleanModel also depends on some function from the CyFluxVizToolbox-v0.03. You should therefore go to the Set Path button at the top of the MATLAB command window, click the button Add with Subfolders..., and select the CyFluxVizToolbox-v0.03 folder, before running cleanModel.m.

To visualize the model, you should open Cytoscape, go to File->Import->Network (Multiple File Types), and select New1122Specific.xml. To layout it properly, install the plugin CySBML, and use it to load the layout file tempLayout.xml. Finally, the plugin CyFluxViz should allow import and visualization of the flux distribution files in 1122FluxMaps.

Finally, the script hostControl simulates what would happen if various coefficients in the biomass reaction were changed, for example if they were lowered to 90, 80, 70%,... of their original values. It calculates the resulting biomass flux and all exchange fluxes under each condition, and prints bar graphs of exchange fluxes for 100 and 30% of the original coefficient values in the folder hostControl.
